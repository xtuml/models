package messagetest.testlibrary;

import java.util.ArrayList;

import lib.BPInteger;
import lib.LOG;
import messagetest.testinterface.ITestInterfaceFromProvider;
import messagetest.testinterface.ITestInterfaceToProvider;
import messagetest.testinterface.ITestReplyFromProvider;
import messagetest.testinterface.ITestReplyToProvider;
...
public class ComponentExecutionTestProvider implements ITestInterfaceToProvider,
		ITestReplyFromProvider {
	// Ports
	private ITestReplyToProvider different = null;
	private ITestInterfaceFromProvider same = null;

	public ComponentExecutionTestProvider(ITestInterfaceFromProvider same,
			ITestReplyToProvider different) {
		this.different = different;
		this.same = same;
	}

	class PTST {
		public Sdt_t test_sdt_a = new Sdt_t();
	...
	public void pingProviderC(ComponentInstance_c sender, int simpleBV_p,
		...
		test_sdt_temp.array_m.add(0, test_sdt_temp.array_m.get(0) + 1);
		test_sdt_temp.sdt_m.simple_m = test_sdt_temp.sdt_m.simple_m + 1;
		if (test_sdt_temp.sdt_m.string_m != "pingRequirer") {
			LOG.LogFailure("Unexpected value in pingProviderC: >"
					+ test_sdt_temp.sdt_m.string_m + "<");
		}
		test_sdt_temp.sdt_m.string_m = "pingProviderC";
		ArrayList<Integer> test_array_temp = new ArrayList<Integer>(
				arrayBV_p);
		test_array_temp.add(0, test_array_temp.get(0) + 1);
		...
		if (ptest != null) {
			same.pingRequirerC(ptest.test_sender, simpleBV_p + 1,
					test_array_temp, test_sdt_temp, simpleBR_p,
					test_array_temp, test_sdt_temp, name);
	...
	public void pingProviderSig(ComponentInstance_c sender, int simple_p,
		...
		Sdt_t test_sdt_temp = sdt_p;
		test_sdt_temp.simple_m = test_sdt_temp.simple_m + 1;
		test_sdt_temp.array_m.add(0, test_sdt_temp.array_m.get(0) + 1);
		test_sdt_temp.sdt_m.simple_m = test_sdt_temp.sdt_m.simple_m + 1;
		if (!test_sdt_temp.sdt_m.string_m.equals("pingRequirerSig")) {
			LOG.LogFailure("Unexpected value in pingProviderSig: >"
					+ test_sdt_temp.sdt_m.string_m + "<");
		}
		test_sdt_temp.sdt_m.string_m = "pingProviderSig";
	...
	public void pingProviderSigS(ComponentInstance_c sender, int simple_p,
		...
		test_sdt_temp.array_m.add(0, test_sdt_temp.array_m.get(0) + 1);
		test_sdt_temp.sdt_m.simple_m = test_sdt_temp.sdt_m.simple_m + 1;
		test_sdt_temp.sdt_m.string_m = "pingProviderSigS";
	...
	public void pingProviderSigC(ComponentInstance_c sender, int simple_p,
		...
		CorePlugin.out.println("Test Class dequeueing. Event: same::pingProviderSigC (simple_p:integer, array_p:integer, sdt_p:Sdt_t, name:string)");
	...
	public void executeOpTest(ComponentInstance_c sender, String name) {
		...
		testClass.test_sdt_a.sdt_m = new Simple_sdt_t();
		...
		Sdt_t sdtResult = same.getSDTFromRequirer(null, "execute");
