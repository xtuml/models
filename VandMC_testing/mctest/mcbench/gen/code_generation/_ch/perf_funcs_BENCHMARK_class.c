...
perf_funcs_BENCHMARK_op_generate_and_dispatch_event( const i_t p_count, const i_t p_duration, const i_t p_phase, const i_t p_testnum )
{
  ...
  if ( 1 == p_phase ) {
    ...
    odometer_reading = ( ((perf_funcs_A *)xtUML_detect_empty_handle( a, "A", "a.odometer" ))->odometer * p_count );
...
perf_funcs_BENCHMARK_op_select_any_where_10000( const i_t p_count, const i_t p_duration, const i_t p_phase, const i_t p_testnum )
{
  ...
  if ( 1 == p_phase ) {
    ...
    while ( 0 < i ) {
      ...
      a = (perf_funcs_A *) Escher_CreateInstance( perf_funcs_DOMAIN_ID, perf_funcs_A_CLASS_NUMBER );
      /* ASSIGN a.ID = i */
      ((perf_funcs_A *)xtUML_detect_empty_handle( a, "A", "a.ID" ))->ID = i;
      /* ASSIGN i = ( i - 1 ) */
      i = ( i - 1 );
    }
  }
    ...
    while ( i > 0 ) {
      ...
      { perf_funcs_A * selected;
        ...
        while ( (selected = (perf_funcs_A *) Escher_IteratorNext( &iteraperf_funcs_A )) != 0 ) {
          if ( ((perf_funcs_A *)xtUML_detect_empty_handle( selected, "A", "SELECTED.ID" ))->ID == i ) {
            a = selected;
            break;
          }
        }
...
perf_funcs_BENCHMARK_op_read_attribute( const i_t p_count, const i_t p_duration, const i_t p_phase, const i_t p_testnum )
{
  ...
  if ( 1 == p_phase ) {
    ...
    while ( i > 0 ) {
      i_t j;
      /* ASSIGN j = a.odometer */
      j = ((perf_funcs_A *)xtUML_detect_empty_handle( a, "A", "a.odometer" ))->odometer;
      /* ASSIGN j = a.odometer */
      j = ((perf_funcs_A *)xtUML_detect_empty_handle( a, "A", "a.odometer" ))->odometer;
      /* ASSIGN j = a.odometer */
      j = ((perf_funcs_A *)xtUML_detect_empty_handle( a, "A", "a.odometer" ))->odometer;
...
perf_funcs_BENCHMARK_act1( perf_funcs_BENCHMARK * self, const Escher_xtUMLEvent_t * const event )
{
  ...
  ((perf_funcs_BENCHMARK *)xtUML_detect_empty_handle( self, "BENCHMARK", "self.test_number" ))->test_number = ( ((perf_funcs_BENCHMARK *)xtUML_detect_empty_handle( self, "BENCHMARK", "self.test_number" ))->test_number + 1 );
  /* TESTLOOP::runtest(duration:self.duration, testnum:self.test_number) */
  perf_funcs_TESTLOOP_runtest( ((perf_funcs_BENCHMARK *)xtUML_detect_empty_handle( self, "BENCHMARK", "self.duration" ))->duration, ((perf_funcs_BENCHMARK *)xtUML_detect_empty_handle( self, "BENCHMARK", "self.test_number" ))->test_number );
}
